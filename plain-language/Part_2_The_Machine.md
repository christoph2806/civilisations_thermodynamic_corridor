...
The essay defines a quantity **$\Sigma$** (sigma)—the *assembly stock*—via four simple rules:
...
3. **Decay without upkeep:** If we stop maintaining, $\Sigma$ shrinks. Steel corrodes, concrete spalls, code rots. The rate of shrinkage is proportional to how much we have: $\frac{d\Sigma}{dt} = C(t) - \delta\Sigma$. Construction $C(t)$ adds; decay $\delta\Sigma$ subtracts ($\delta$ = decay rate, fraction of stock lost per year).

4. **What counts:** Only things maintained by civilisation's energy flows. The test: if civilisation's power went to zero, would it fall apart on a human timescale? If yes, it's in $\Sigma$. If no (e.g. a wild forest maintained by sunlight), it's not.

### Technosphere vs biosphere vs geosphere

- **Geosphere:** Rock, ocean, atmosphere. Structure from geology, not civilisation. Not part of $\Sigma$.
- **Biosphere:** Living things. Huge assembly (a single cell has thousands of "assembly steps"). But maintained by photosynthesis and ecology, not by civilisation. Not part of $\Sigma$.
- **Technosphere:** Everything we build and maintain—roads, buildings, servers, farms that depend on our energy. This is $\Sigma$.
...
$\Sigma$ decays at rate $\delta$ (roughly 2–5% per year for the US technosphere). To hold $\Sigma$ constant, we must supply power. The *minimum* power is:
...
Real civilisation also builds, transports, computes, governs, and grows. Total power $P$ exceeds the maintenance floor by a factor $\mu$ (the *metabolic multiplier*), typically ~1.7–2.5. So:
...
### The four components of $\Gamma$

Each has a different "status":

- **$\delta$ (decay rate):** Physics says $\delta > 0$. Engineering can lower it (build more durable stuff). But the trend has been toward *higher* $\delta$—more electronics, software, short-lived goods—which raises the burden.

- **$\xi$ (energy cost per repair):** Always positive. Varies by material and process.

- **$\eta$ (efficiency):** Always $< 1$. We waste most of the energy we use. Current aggregate ~10–15%.

- **$\mu$ (metabolic multiplier):** Ratio of total power to maintenance power. Partly institutional (interest rates, military spending, etc.)—reformable. Partly evolutionary (competitive pressure to grow)—harder to change.
...
So if $P_{\mathrm{civ}}$ grows, $T$ must rise. No technology, fuel, or policy changes this. It's geometry and radiative physics.
...
If $T$ must stay below some habitability limit $T_{\mathrm{hab}}$, then there's a maximum power we can dissipate:
...
Information isn't free. Landauer (1961) showed that *erasing* a bit—the irreversible step in computation—must dissipate at least $k_{\mathrm{B}} T \ln 2$ of energy per bit. At room temperature, that's ~3×10⁻²¹ J per bit. This has been experimentally confirmed (Bérut et al. 2012, others). It's a fundamental limit, like the Carnot limit for heat engines.
