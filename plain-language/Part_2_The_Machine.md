...
3. **Decay without upkeep:** If we stop maintaining, Σ shrinks. Steel corrodes, concrete spalls, code rots. The rate of shrinkage is proportional to how much we have: **dΣ/dt = C(t) − δΣ**. Construction C(t) adds; decay δΣ subtracts (δ = decay rate, fraction of stock lost per year).
...
where δ = decay rate, ξ = energy cost per "assembly step" restored, η = conversion efficiency (currently ~10–15%), Σ = assembly stock. The product δξ/η is always positive. No technology can make it zero. This is the *maintenance floor*: the irreducible power needed just to prevent things from falling apart.
...
The composite **Γ** (gamma) = μδξ/η captures how much power we need per unit of assembly stock. Here μ = metabolic multiplier (total power / maintenance power), δ = decay rate, ξ = exergy cost per repair, η = efficiency. It depends on technology, materials, and institutions—but it's always positive and bounded below.
...
**Bottom line:** The ceiling argument only needs that Γ > 0. The exact value affects *when* we hit the ceiling, not *when* it exists.
...
Every watt we use ends up as heat. Nuclear, wind, solar, fusion—all of it. That heat must be radiated to space. The rate is set by the Stefan–Boltzmann law: **P_rad = εσAT⁴**. At steady state:
...
- **Floor:** P ≥ (δξ/η)·Σ (δ = decay rate, ξ = exergy cost, η = efficiency). Below this, Σ decays.
- **Ceiling:** Σ ≤ Σ_max. Above this, the planet overheats.
...
**Pattern:** Efficiency gains get recycled into more consumption. The power–assembly coupling Γ doesn't care whether the assembly is in steel mills or server farms. It couples to the total. The information economy doesn't escape the constraint—it accelerates toward it.
